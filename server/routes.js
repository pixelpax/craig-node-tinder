...
async function fetchSprinterPostsFromCity(city) {
  ...
  try {
    listings = await client.search({
      city,
      category: 'cta'
    }, 'sprinter -extended -"170 wb" -"170 wheelbase"');
    return listings.filter(listing => !!listing.title.toLowerCase().match(/sprinter/))
      .filter(listing => !listing.title.toLowerCase().match(/(extended)|(170.?(wb|wheelbase))/))
  } catch (e) {
    throw e;
    // res.status(404);
...
async function fetchTransitPostsFromCity(city) {
  ...
  try {
    ...
    return listings.filter(listing => !!listing.title.toLowerCase().match(/transit/))
  } catch (e) {
    throw e;
    // res.status(404);
    // res.send('Couldnt pull postings from clist');
  ...
  app.post('/fetch-posts', async (req, res) => {
    ...
    try {
      ...
      if (failedCities.length) {
        res.send('Cities failed: ' + failedCities.toString());
      }
      res.sendStatus(200);
